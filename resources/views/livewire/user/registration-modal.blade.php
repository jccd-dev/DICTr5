...
{{--                                        <x-forms.file name="Upload Certificate" placeholder="Upload Certificate" model="trainings.{{ $i }}.certificate" id="certificate-{{ $i }}" accept=".pdf,.doc,.docx,image/*" classes="mb-3 md:mb-6 flex-1 flex-col" />--}}
            ...
            <div x-show="state == 3">
                ...
                <div x-show="currentStatus == 'professional'">
                    ...
                    <div class="flex md:flex-row flex-col w-full gap-3">
                        ...
                        <div class="flex flex-col basis-1/4 gap-3">
                            ...
                            @error('officeCategory') <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div class="flex md:flex-row flex-col w-full gap-3">
                        <div class="flex md:flex-row flex-col basis-3/4 gap-3">
                ...
                <div class="w-full md:w-3/4 lg:w-1/2">
                    ...
                    @error('pl') <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p> @enderror
                </div>
                <br />
                <hr />
            </div>
            <div x-show="state == 4">
                <div x-show="currentStatus == 'student'">
                    ...
                    <div class="w-full md:w-4/5 flex gap-3">
                        <x-forms.file name="Upload Passport Size Image" placeholder="Upload Passport Size Image" model="passport" id="passport" accept=".pdf,.doc,.docx,image/*" classes="mb-3 md:mb-6 flex-1 flex-col" />
                        <x-forms.file name="Diploma / TOR" placeholder="Diploma / TOR" model="psa" id="psa" accept=".pdf,.doc,.docx,image/*" classes="mb-3 md:mb-6 flex-1 flex-col" />
                    </div>
                    <div class="w-full md:w-2/5 flex gap-3">
                        <x-forms.file name="COE / COG" placeholder="COE / COG" model="cert" id="certs" accept=".pdf,.doc,.docx,image/*" classes="mb-3 md:mb-6 flex-1 flex-col" />
                    </div>
                </div>
                <div x-show="currentStatus == 'professional'">
                    <h1 class="font-bold font-quicksand text-xl flex self-start my-4">Required Documents</h1>
                    <div class="w-full md:w-4/5 flex gap-3">
                        <x-forms.file name="Valid ID" placeholder="Valid ID" model="validId" id="valid-id" accept=".pdf,.doc,.docx,image/*" classes="mb-3 md:mb-6 flex-1 flex-col" />
                        <x-forms.file name="Diploma / TOR" placeholder="Diploma / TOR" model="diploma" id="diploma" accept=".pdf,.doc,.docx,image/*" classes="mb-3 md:mb-6 flex-1 flex-col" />
                    </div>
                </div>
                <br />
                <hr />
                ...
                <div class="flex flex-col justify-center items-center my-5">
                    ...
                    @error('signature') <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p> @enderror
                </div>
