<div class="font-quicksand">
            ...
            @else
                  <div class="relative overflow-x-auto">
                      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                          ...
                          <tbody>
                          @forelse($sent_emails as $sent_email)
                              <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                  <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                      {{$sent_email->user}}
                                  </th>
                                  ...
                                  <td class="px-6 py-4">
                                      {{date('M d, Y | h:i A', strtotime($sent_email->timestamp))}}
                                  </td>
                                  <td class="px-6 py-4">
                                      {{$sent_email->intended_for}}
                                  </td>
                                  <td class="px-6 py-4">
                                      <div wire:click="prepare_to_delete_sent_email({{$sent_email->id}})">
                                          <svg fill="none" class="w-5 text-right cursor-pointer text-red-700" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                              <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"></path>
                                          </svg>
                                      </div>
    ...
    {{-- Delete Modal for Sent Email --}}
    <x-modal wire:model.defer="sent_email_modal" blur="true" align="center" max-width="sm">
        <x-card title="Delete Confirmation">
            <p class="text-gray-600">
            ...
            <x-slot name="footer">
                <div class="flex justify-end gap-x-4">
                    <x-button secondary label="Cancel" x-on:click="close" />
                    <x-button wire:click="delete_sent_email" red label="Yes, delete it!" />
                </div>
            </x-slot>
        </x-card>
    </x-modal>
